...
int main(int argc, char **argv) {
    ...
    if (with_files) {
        ...
        pipef=(int**)malloc(sizeof(int*) * pnum);
    }
    ...
    for (int i = 0; i < pnum; i++) {
        if (!with_files){
            pipef[i]=(int*)malloc(sizeof(int)*2);
            if (pipe(pipef[i])==-1) {
        ...
        if (child_pid >= 0) {
            ...
            if (child_pid == 0) {
                ...
                if (with_files) {
                    fwrite(&min_max_i.min,sizeof(int),1,f);
                    fwrite(&min_max_i.max,sizeof(int),1,f);
                } else {
                    close(pipef[i][0]);
                    write(pipef[i][1],&min_max_i.min,sizeof(int));
                    write(pipef[i][1],&min_max_i.max,sizeof(int));
                    close(pipef[i][1]);
                }
                return 0;
            }
    ...
    for (int i = 0; i < pnum; i++) {
        ...
        if (with_files) {     // read from files
            ...
            close(pipef[i][1]);
            read(pipef[i][0],&min,sizeof(int));
            read(pipef[i][0],&max,sizeof(int));
            close(pipef[i][0]);
            free(pipef[i]) ;
        }
